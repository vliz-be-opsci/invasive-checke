...
def process_input_row(row):
    ...
    if row["isNegativeControlGene"]:
        log.warning(
            (
                "Sample is Negative Control: "
                "not bothering with invasiveness check..."
            )
        )
    elif aphia_id is not None:
        results, invasive_df = invasive_checker.check_aphia(
            lon, lat, aphia_id, source="worms"
        )
...
def do_work(input_file, output_folder, meta_file, cfg):
    ...
    with open(
        f"{output_folder}/invasive_checker.log", "w", encoding="utf8"
    ) as f:
        with open(input_file, "rb") as fi:
            f.write(
                f"{md5(fi.read()).hexdigest()}\t(md5sum of {input_file})\n"
            )
    ...
    worms_df_unpivot = pd.melt(
        worms_df,
        id_vars=["classification", "OTU"],
        var_name="AccessionID",
        value_name="Count",
    )
    meta_df = pd.read_csv(meta_file)
    sample_df = utils.get_sample_location_df(
        worms_df_unpivot.AccessionID.unique(), meta_df
    )
    worms_df_unpivot = pd.merge(
        worms_df_unpivot,
        sample_df,
        how="left",
        left_on="AccessionID",
    ...
    wrims_df = worms_df_unpivot.apply(
        lambda row: process_input_row(row), axis=1
    )
    ...
    aphia_df = pd.merge(
        worms_df,
        wrims_df[["classification", "Aphia_ID"]],
        how="left",
        on="classification",
    )
    ...
    if aphia_df["OTU"].is_unique:
        log.info(
            "Writing worms.csv classification file to {0}".format(
                aphia_filepath
            )
        )
        ...
        log.warning(
            "Non-unique OTU column. Will add suffix to duplicate OTUs..."
        )

        aphia_df["duplicate_otu"] = aphia_df["OTU"].duplicated()
        aphia_df["duplicate_count"] = (
            aphia_df.groupby(aphia_df.OTU).cumcount().values
        )
        aphia_df["OTU"] = aphia_df.apply(
            lambda row: row["OTU"] + "_" + str(row["duplicate_count"])
            if row["duplicate_otu"]
            else row["OTU"],
            axis=1,
        )
        aphia_df.drop(
            ["duplicate_otu", "duplicate_count"], axis=1, inplace=True
        )
        log.info(
            "Writing worms.csv classification file to {0}".format(
                aphia_filepath
...
# take a dataframe and change several column names to match column names
...
def clean_up_dataframes(df, cfg):
    ...
    df.rename(
        columns={class_col_name: "classification", otu_col_name: "OTU"},
        inplace=True,
    )
...
if __name__ == "__main__":
    ...
    """
    PARSER = argparse.ArgumentParser(description="Invasive Checker")
    PARSER.add_argument(
        "-l",
        "--loglevel",
        default="INFO",
    ...
    PARSER.add_argument("-i", "--input_file", help="Path to input csv file.")
    PARSER.add_argument(
        "-m", "--meta_file", help="Path to sample metadata csv file."
    )
    PARSER.add_argument(
        "-o",
        "--output_folder",
