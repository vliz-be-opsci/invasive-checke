...
def derive_status(this_aphia_df):
    ...
    There might be some issues with this:
      ...
      - What happens if the only marineregion is large?
            Something like "Atlantic Ocean"
      - How do we say that a sample is unrecorded at a marineregion?
            How do we find "missing" marineregions?
    """
...
def check_aphia(lon, lat, id, source="worms"):
    """
    Check if the aphia is invasive, native or unknown.
        Return MRGID's that are within <buffer> degrees
        of the sample location.

    CRS for everything is WGS84.

    aphia_df is a dataframe of MRGID's and status' that are associated with
        the aphia_id. Retrieved from WRIMS
    ...
    # Get Aphia status, geoms associated with the aphia and whether the
    #   sample location is inside those geoms or not.
    # =============
    if source == "worms":
        aphia_id = id
    ...
    log.debug("  -Finding MarineRegions that intersect with sample...")
    ...
    # this_aphia_gdf['distance_to_distribution'] =
    #   this_aphia_gdf.distance(sample_point)
    this_aphia_df.establishmentMeans.replace(
        "Alien", "Introduced", inplace=True
    )
    this_aphia_df.establishmentMeans.fillna("Recorded", inplace=True)

    # =============
    # Apply some human logic to determine whether the above results
    #   are of interest or not
    # =============
    log.debug("  -Applying logic...")
    ...
    # =============
    # condition = this_aphia_gdf['distance_to_distribution'] < buffer
    # this_aphia_gdf = this_aphia_gdf[condition]
    invasive_df = this_aphia_df.drop(
        [
            "decimalLongitude",
            "decimalLatitude",
            "higherGeography",
...
def get_external_status(external_id, id_source):
    ...
    try:
        aphia_url = (
            "https://marinespecies.org/rest/"
            f"AphiaRecordByExternalID/{external_id}?type={id_source}"
        )
        ...
        log.warning(
            f"Error retrieving distribution for {id_source}: {external_id}"
        )
        log.warning(err)
        return None
...
def get_aphia_status(aphia_id):
    """
    Get the MRGIDs and invasive status for the aphia_id specified.
        Return dataframe with MRGID's of known distribution and the
        native/alien status of the MRGID/APHIA pair.
        Good test values are aphiaID = 107451 (chinese mitten crab, invasive)
    """
    wrms_distribution = (
        "http://www.marinespecies.org/rest/"
        f"AphiaDistributionsByAphiaID/{aphia_id}"
    )
    try:
        ...
        if len(wrms_dist) == 0:
            ...
            wrms_dist_df[
                ["root", "MRGID"]
            ] = wrms_dist_df.locationID.str.rsplit("/", 1, expand=True)
...
def get_aphia_from_lineage(tax_string, sep=";"):
    """
    Given a taxon lineage string
        (Eukaryota;Chordata;Ascidiacea;
            Enterogona;Ascidiidae;
            Ascidiella;Ascidiella scabra)
...
def get_aphia_from_taxname(taxa_name):
    ...
    https://www.marinespecies.org/rest/
        AphiaRecordsByMatchNames?scientificnames[]=<some name>&marine_only=true
    """
    taxamatch_url = (
        "https://www.marinespecies.org/rest/"
        "AphiaRecordsByMatchNames?scientificnames[]"
        f"={taxa_name}&marine_only=true"
    )
    try:
...
def get_mrgid_from_latlon(lat, lon):
    """
    Given a the location of a sample, find the Marineregions
        that intersect with it.
    """
    mr_url = (
        "https://www.marineregions.org/rest/"
        f"getGazetteerRecordsByLatLong.json/{lat}/{lon}/?offset=0"
    )
    try:
